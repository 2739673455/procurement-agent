...
| 财务管理     | 预算与占用、履约发票匹配、支付计划、预付核销、付款申请授权及实付登记 | 预算分析、发票差异解释、付款状态查询 |
| 库存与仓储   | 物料、仓库、期初库存、入出库流水、收货、退货                         | 库存参考、采购需求和交付分析         |

正式业务记录与审批由业务后端管理，Agent 服务保存对话、任务和分析记录。业务平台在 Agent 服务不可用时仍能完成人工采购流程。

财务模块首期定位为采购预算与应付管理，不建设完整会计核算或银行支付系统；付款状态来自财务人员登记及凭证，区分已确认实付与待补正/待核实款；真实出款不因复核退回而释放支付额度。库存必须记录出入库与调整，不能只累计采购收货。

采购流程参考 IBM TRIRIGA 11.6，支持协议下单、RFQ、RFP 和招标分支，合同、协议和变更由采购模块管理，按业务切片实施。签署与供应商往来首期在线下完成并登记凭证。不建设独立合同、邮件或办公系统，不接入外部供应商及市场数据源；自动合同审查、自动续签、舆情监控、市场价格预测和自动谈判不纳入本期范围。详细来源及本地化决定见[采购业务平台详细设计](platform/docs/DESIGN.md)。
...
预付采购可在采购承诺生效并满足批准的支付计划条件后先付款，后续履约和发票确认后核销预款，再支付尾款；不要求先做虚假验收。

采购主流程按[平台 README](platform/README.md#逐步执行流程)的角色动作执行：提出需求、申请审批、分配采购任务与核实需求、选择采购方式、形成统一采购订单（按需引用合同）、履约验收、发票核对、付款申请与授权、实际付款登记及收尾。

协议下单跳过本轮竞争寻源；RFQ 将询价与报价分开；RFP 和招标分别记录征集文件、响应、评审与授予。合同约束条款，全部采购由订单明细承载唯一预算承诺；固定范围合同自动派生订单并复用审批，实物与非实物分别使用合格收货和已确认业务验收依据。实物到货待检、当场拒收及未入库退回不计入可用库存或预算执行；非实物验收按金额和风险配置独立审批，明确免审批规则除外。

助手可辅助需求检查、内部供应商推荐、响应比较、异常解释及草稿准备。审批、合同签署、正式验收、支付授权和财务复核由授权人员完成；助手不可用时，人工仍可完整执行。供应商首期线下参与，由采购员登记往来和原始凭证。

业务对象、状态、配套动作和实施顺序见[采购业务平台详细设计](platform/docs/DESIGN.md)。数据库 v3 已补齐申请预算、履约检验、联动变更、付款及预付核销、责任评价字段；当前为设计成果，尚未实现业务代码或迁移。
